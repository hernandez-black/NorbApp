...
export default function Vehiculos() {
  ...
  const handleGuardar = () => {
    if (!form.marca.trim() || !form.modelo.trim() || !form.placas.trim() || !form.cliente_id) {
      showToast("Marca, modelo, placas y cliente son obligatorios", "error");
      return;
    }
    if (editando) {
  ...
  const handleEliminar = (id: string, nombre: string) => {
    setModalConfig({
      ...
      message: `¿Estás seguro de eliminar el vehículo ${nombre}? Esta acción no se puede deshacer.`,
      type: 'danger',
      onConfirm: () => {
        setVehiculos(vehiculos.filter((x) => x.id !== id));
        showToast(`Vehículo ${nombre} eliminado`, 'info');
      },
    });
  };
                    ...
                    <tr key={v.id}>
                      ...
                      <td>
                        <div className="acciones">
                          ...
                          <button className="btn-icon btn-eliminar" title="Eliminar" onClick={() => handleEliminar(v.id, `${v.marca} ${v.modelo}`)}><FaTrash /></button>
                        </div>
                      </td>
                    </tr>
                  );
